...
class HEPaS1(object):
    def login(self, student_id, first_name, last_name, oust_email):
        if student_id != "" and first_name != "" and last_name != "" and oust_email != "":
            if server2.retrieve_user_from_table(student_id, first_name, last_name, oust_email):
                print("Login Success")
                return True
            else:
                print("Login Failed")
    ...
    def evaluate_existing_records(self, student_id):
        result = server2.get_existing_score(student_id)
    ...
    def retrieve_existing_records(self, student_id):
        return server2.get_existing_records(student_id)

    def add_unit_record(self,student_id, unit_code, score):
        return server2.add_new_record(student_id, unit_code, score)
def setup_server():
