    ...
    if auth:
        ...
        if student_status == "yes":
            while True:
                ...
                print("4. Enter new scores for evaluation")
                ...
                if choice == "1":
                    ...
                    records = hepas_proxy.get_existing_records(student_id)
                    if isinstance(records, str):
                        print(records)
                    else:
                        for record in records:
                            print(f"Course: {record['course_code']}, Grade: {record['grade']}")
                elif choice == "3":
                    ...
                    while True:
                        ...
                        try:
                            ...
                            score = float(score_str.strip())
                            if 0 <= score <= 100:
                                response = hepas_proxy.add_new_record(student_id, unit_code, score)
                                print(response)
                            else:
                                print("Please enter a valid score (0 to 100)")
                        except ValueError:
